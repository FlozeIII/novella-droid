...
import type { ShelfBookRef } from '@novella/client-core';
...
export function useBookDetail(
  ...
  const shelfRef: ShelfBookRef = useMemo(
    () => ({ id: bookId, type: type === 'Comic' ? 'COMIC' : 'NOVEL' }),
    [bookId, type],
  );
    ...
    try {
      const retainedSeriesTitle = normalizedSeriesTitleHint
        ?? (stateRef.current.status === 'ready' && stateRef.current.book.id === bookId
          ...
          : null);
      const [serverBook, cachedPosition] = await Promise.all([
        (type === 'Comic' ? comicDetails : bookDetails).load(bookId),
        getCachedReaderPosition(bookId),
      ]);
      // Shelf membership is a separate concern from reading the book: a shelf
      // failure must degrade to an inline message, never hide the book.
      let isInShelf = false;
      let shelfError: BookUserMessage | null = null;
      try {
        isInShelf = await shelf.contains(shelfRef);
      } catch (error) {
  ...
  }, [bookId, normalizedSeriesTitleHint, shelfRef, type]);
  ...
  const toggleShelf = useCallback(async () => {
    ...
    try {
      const isInShelf = await shelf.toggleBook(shelfRef);
      setState((current) =>
        current.status === 'ready'
