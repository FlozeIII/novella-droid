...
export function readiumLocatorToReaderPosition(
  ...
  if (blocks.length === 0 || !readiumHrefNamesChapter(locator.href, chapterId)) return null;
...
/**
 * True when `href` names this chapter's document.
 *
 * The canonical spelling is the root-relative one this module emits
 * (`EPUB/chapters/1.xhtml`), but a navigator that keeps the OPF's own spelling
 * reports `chapters/1.xhtml` — relative to the package document at
 * `EPUB/package.opf`. Accept either, which is the same tolerance the reader's link
 * handling applies when it matches `(?:^|\/)chapters\/(\d+)\.xhtml$`.
 *
 * Comparisons are anchored on whole path segments, so chapter 1 cannot be confused
 * with chapter 11.
 */
function readiumHrefNamesChapter(href: unknown, chapterId: number): boolean {
  const normalized = normalizeReadiumHref(href);
  if (normalized === null) return false;
  const expected = readiumChapterHref(chapterId);
