...
/** Directory, inside EPUB/, holding one font file per chapter payload. */
export const READIUM_FONTS_HREF = 'fonts';
...
export interface ReadiumChapterDocumentOptions {
  ...
  /** Font for this chapter's payload, referenced from the document when present. */
  bookFont?: { family: string; revision: string } | null;
  chapterId: number;
...
/** EPUB-relative href of one chapter payload's font file. */
export function readiumFontHref(revision: string): string {
  return `${READIUM_FONTS_HREF}/${revision}.woff2`;
}

/**
 * Publication-relative path of one chapter payload's font file.
 *
 * Readium's DirectoryContainer only serves the hrefs declared when the
 * publication opens, so this exact string has to reach `declaredHrefs`.
 */
export function readiumFontPublicationHref(revision: string): string {
  return `EPUB/${readiumFontHref(revision)}`;
}
 ...
 *
 * `fontHref` is the target chapter's font (EPUB-relative) and is both declared in
 * the manifest and added to `declaredHrefs`, without which the container refuses
 * to serve it to the WebView.
 */
export function buildReadiumPublicationResources(
  ...
  if (fontHref) {
    manifestItems.push(`<item id="book-font" href="${escapeXml(fontHref)}" media-type="${WOFF2_MEDIA_TYPE}"/>`);
  }
  ...
  const resources: Record<string, string> = {
    ...
    [`EPUB/${READIUM_STYLESHEET_HREF}`]: buildReadiumPublicationStylesheet(),
  };

  const declaredHrefs = [
    ...Object.keys(resources),
    ...ordered.map((chapter) => `EPUB/${chapterHrefFor(chapter.id)}`),
    // The target's font is the only one whose revision is known this early, and
    // Readium's container serves nothing that was not declared when it opened.
    ...(fontHref ? [`EPUB/${fontHref}`] : []),
  ];
...
export function buildReadiumChapterDocument({
  ...
  const fontClass = bookFont ? ' class="nv-book-font"' : '';
  ...
  return [
    ...
    buildChapterFontStyle(bookFont),
...
/** Only metadata and the target chapter gate first paint. */
...
/**
 * The font belongs to one chapter payload, never to the publication: the
 * backend issues a fresh font (at a stable URL) for every rescrambled chapter,
 * so a shared `@font-face` would decode later chapters with an earlier font.
 *
 * The source is a file named after the payload's revision rather than an inlined
 * data URL: inlining put ~1.4MB of base64 in the document and kept the first
 * paint blank for the better part of a minute on device, while the same chapter
 * without it paints in seconds. The per-revision name is what keeps a WebView
 * from serving a font decoded for a different scramble; these documents live in
 * EPUB/chapters/, so the href is relative to that.
 */
function buildChapterFontStyle(
  bookFont: ReadiumChapterDocumentOptions['bookFont'],
): string {
  if (!bookFont) return '';
  const family = escapeXml(bookFont.family);
  return [
    '<style type="text/css">',
    `@font-face{font-family:'${family}';font-display:block;`,
    `src:url("../${readiumFontHref(bookFont.revision)}") format('woff2');font-style:normal;font-weight:400;}`,
    `body.nv-book-font{font-family:'${family}',sans-serif;}`,
    '</style>',
  ].join('');
}

function buildReadiumPublicationStylesheet(): string {
  return [
