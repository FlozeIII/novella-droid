...
test('an OPF-relative href maps like the canonical one', () => {
  // A navigator that keeps the package document's own spelling reports
  // `chapters/42.xhtml`, relative to `EPUB/package.opf`, rather than the
  // root-relative href this module emits.
  const position = readiumLocatorToReaderPosition({
...
test('an OPF-relative href for another chapter is still rejected', () => {
  const position = readiumLocatorToReaderPosition({
...
test('a chapter-number prefix does not match a longer chapter number', () => {
  const position = readiumLocatorToReaderPosition({
    href: 'EPUB/chapters/11.xhtml',
    type: 'application/xhtml+xml',
    locations: { progression: 0 },
  }, 1, blocks);
