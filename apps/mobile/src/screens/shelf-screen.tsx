...
function ShelfContent({
  ...
  const { t } = useTranslation('library');
  const styles = useShelfScreenStyles();
  // Keyed by typed shelf ref, not by id alone: a novel and a comic can share an id.
  const booksByKey = new Map(
    snapshot.books.flatMap((record) => record.book
      ? [[shelfBookRefKey(record.ref), record.book] as const]
      : []),
  );
  ...
  const renderShelfItem = (item: ShelfItem) => {
    ...
    if (item.type === 'FOLDER') {
      ...
      const previewBooks = snapshot.items
        ...
        .map((child) => booksByKey.get(shelfItemKey(child)))
