...
export function ReaderScreen({ bookId, sortNum, openPosition = 'saved' }: ReaderScreenProps) {
  ...
  const readerFont = useReaderFont(content?.chapter);
  ...
  const publication = useReadiumPublication({
    bookFont: readerFont.font,
    bookId,
    content,
    fontRequired: requiresReaderFont,
        ...
        {requiresReaderFont && readerFont.status === 'error' ? (
          ...
          <View style={styles.reader}>
            <NovellaReadiumView
              ...
              onError={(error) => {
                // TEMP diagnostic: the error state below renders only `message`, so the
                // native `code` never reaches the screen or any log.
                console.log('[novella-readium] error', error);
                setNativeError({ kind: 'raw', text: error.message });
              }}
              ...
              onStatus={(status) => {
                // TEMP diagnostic: status is otherwise discarded, which makes a resource
                // that fails to load invisible everywhere.
                console.log('[novella-readium] status', status);
              }}
