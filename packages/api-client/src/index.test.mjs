...
test('round-trips current mixed shelf items with the canonical casing', async () => {
  const calls = [];
  const client = new ApiClient(
    { async request() { throw new Error('not used'); } },
    {
  ...
  // The port echoes back the structural version the server handed it rather
  // than forcing upstream's canonical constant (see the shelf model sync notes).
