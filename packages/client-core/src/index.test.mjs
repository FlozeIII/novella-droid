...
test('shelf repository publishes one shared typed snapshot and echoes the server version', async () => {
  const saved = [];
  const useCase = createShelfUseCase({
    async getBookShelf() {
      return {
  ...
  assert.equal(published.length, 2);
  // Deliberate deviation from upstream: this port echoes back the structural
  // version the server handed it instead of forcing its own canonical constant.
  assert.equal(saved[0].version, '20220211');
...
test('failed shelf save protects the optimistic projection until a complete retry succeeds', async () => {
  ...
  assert.equal(refreshed.version, 'stale-server');
...
test('typed contains and toggle distinguish Novel and Comic with the same id', async () => {
  ...
  assert.equal(savedDrafts.every((draft) => draft.version === '20220211'), true);
});
